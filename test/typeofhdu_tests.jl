@testset "typeofhdu" begin
    ...
    cards = Card[]
    data  = nothing
    mankeys, reskeys = FITS.get_reserved_keys(cards)
    @test FITS.typeofhdu(data, mankeys) == Primary
    ...
    mankeys, reskmeys = FITS.get_reserved_keys(cards)
    @test FITS.typeofhdu(data, mankeys) == Random
    ...
    # mankeys, reskeys = FITS.get_reserved_keys(cards)
    # @test FITS.typeofhdu(data, mankeys) == Image
    ...
    data = ["col1  col2  col3  col4",
            "col1  col2  col3  col4"]
    mankeys, reskeys = FITS.get_reserved_keys(cards)
    @test FITS.typeofhdu(data, mankeys) == Table

    #  test Bintable type
    cards = Card[]
    data = [(a=1, b=1., c="1"), (a=2, b=2., c="2"), (a=3, b=3., c="3")]
    mankeys, reskeys = FITS.get_reserved_keys(cards)
    @test FITS.typeofhdu(data, mankeys) == Bintable
    ...
    data = nothing
    mankeys, reskeys = FITS.get_reserved_keys(cards)
    @test FITS.typeofhdu(data, mankeys) == ZImage
    ...
    data = nothing
    mankeys, reskeys = FITS.get_reserved_keys(cards)
    @test FITS.typeofhdu(data, mankeys) == ZTable
    ...
    data = nothing
    mankeys, reskeys = FITS.get_reserved_keys(cards)
    @test FITS.typeofhdu(data, mankeys) == Conform
